...
def get_user_read_only_data(title_id):
    url = f"https://{title_id}.playfabapi.com/Client/GetUserReadOnlyData"
    headers = {
    ...
    if response.status_code != 200:
        ...
        print(catalog)
        filename = f"stuff/{title_id}_stuff.json"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(filename, "w") as outfile:
            json.dump(catalog, outfile)

title_id = globals()['titleid']
get_user_read_only_data(title_id)